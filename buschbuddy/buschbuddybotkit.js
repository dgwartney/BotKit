...
function saleforceAPIInvocation(data,cb){
    ...
    if(data.context.entities.Email){
        ...
        //Risk of sequel injection? No need to apply a filter for sql injection as Kore does it in chatbot.
        var getEmail = {
            method: 'GET',
...
function createCase(data, callback) {
    ...
    // for exception handling if release hits error
    // var webSite = "";
    // if (data.context.createContactId === "NA") {
    //     contactid = data.context.contactId;
    // } else {
    ...
    var dataCaseCreation = {
        ...
        "Product_Information__c": data.context.session.BotUserSession.website,        
...
function gatherFeedback(data,callback){
    ...
    var ratingB;
    if(data.context.entities.FeedbackFirstB==="1"){
        ratingB=1;
    }
    if(data.context.entities.FeedbackFirstB==="2"){
        ratingB=2;
    }
    if(data.context.entities.FeedbackFirstB==="3"){
        ratingB=3;
    }
    if(data.context.entities.FeedbackFirstB==="4"){
        ratingB=4;
    }
    if(data.context.entities.FeedbackFirstB==="5"){
        ratingB=5;
    }
